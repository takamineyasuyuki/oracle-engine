        ...
        if tribe == "ANALYSTS" and is_pc:
            return [
                RoadmapStep(
                    ...
                    description="あなたの論理的思考を活かして、OpenAI API（GPT-5-mini）を使った自動化システムを構築。ココナラで「AI自動化サービス」を出品し、1件1万円×週5件=月20万円を目指します。",
                    tools=["OpenAI API（GPT-5-mini）", "Python3", "VS Code", "ココナラ"],
                    expected_outcome="初月で5〜10万円の収益。自動化システムが24時間働き続ける快感を体験。",
                    detailed_steps=[
                        "platform.openai.com/api-keysをブラウザで開く → 「Create new secret key」ボタンをクリック → APIキー（sk-から始まる文字列）をコピーしてメモ帳に保存",
                        "code.visualstudio.comから VS Codeをダウンロード → インストール → 左サイドバーの「拡張機能」→「Python」を検索してインストール",
                        "新しいファイル「test.py」を作成 → 以下をコピペ：from openai import OpenAI; client=OpenAI(api_key='YOUR_KEY'); response=client.chat.completions.create(model='gpt-5-mini',messages=[{'role':'user','content':'キャッチコピー10個'}]); print(response.choices[0].message.content)",
                        "ココナラで出品：タイトル『AIで100個のコンテンツを一括生成します』→ 価格10,000円 → 納期1日",
                        "依頼が来たらスクリプト実行 → 結果をExcelにコピペ → 納品 → 作業時間10分で1万円！",
                        "月20件受注で200,000円 - ココナラ手数料22%(44,000円) - APIコスト約5,000円 = 純利益約151,000円達成"
                    ],
        ...
        if tribe == "ANALYSTS" and not is_pc:
            return [
                RoadmapStep(
                    ...
                    tools=["ChatGPTアプリ", "Notion", "ココナラアプリ", "Googleスプレッドシート"],
                    expected_outcome="初月で3〜5万円の収益。スマホだけでも論理的に稼げることを実証。",
                    detailed_steps=[
                        "App StoreからChatGPTアプリをダウンロード → Plus（月額約3,400円・税込）に課金 → GPT-5が使えることを確認",
        ...
        if tribe == "DIPLOMATS" and is_pc:
            return [
                RoadmapStep(
                    ...
                    tools=["ChatGPT Plus", "Notion", "Canva", "ココナラ"],
                    expected_outcome="初月で5〜8万円の収益。あなたの共感力がお金に変わる体験。",
                    detailed_steps=[
                        "ChatGPT Plus（月額約3,400円・税込）に課金 → 「あなたは共感力の高いライターです。読者の感情に寄り添った文章を書いてください」というシステムプロンプトを作成",
                        "ココナラで出品：『心に響く自己紹介文・プロフィールを作成します』→ 価格10,000円 → 納期2日",
                        "Canva Pro（月額1,180円 or 年額8,300円）でプロフィール用のビジュアルテンプレートを5種類作成 → セットで差別化",
        ...
        if tribe == "SENTINELS" and is_pc:
            return [
                RoadmapStep(
                    ...
                    tools=["ChatGPT Plus", "Microsoft Office", "ココナラ", "Googleスプレッドシート"],
                    expected_outcome="初月で5〜8万円の収益。正確さが評価され★5レビューが集まる。",
                    detailed_steps=[
                        "ChatGPT Plus（月額約3,400円・税込）に課金 → 「ビジネス文書の校正・品質チェック専門家」としてカスタムGPTを作成",
        ...
        if tribe == "SENTINELS" and not is_pc:
            return [
                RoadmapStep(
                    ...
                    detailed_steps=[
                        "ChatGPTアプリをインストール → Plus課金（月額約3,400円・税込）→ 「データ整理の専門家として正確に作業して」とシステム指示を設定",
